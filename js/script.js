...
let areasDatabase = []; // Nueva variable para áreas desde BD
...
async function cargarRecursosDesdeSupabase() {
    try {
        ...
        // Cargar recursos con JOIN a tabla areas
...
async function cargarAreasDesdeSupabase() {
    try {
        console.log('🔄 [INDEX] Cargando áreas desde Supabase...');
        ...
        console.log('✅ [INDEX] Áreas cargadas desde BD:', {
            total: areasDatabase.length,
            fuente: 'Supabase',
            areas: areasDatabase.map(a => ({
                nombre: a.nombre,
                correo: a.correo,
                orden: a.orden
            }))
        ...
        // Si no hay áreas en BD, usar fallback hardcoded
        if (areasDatabase.length === 0) {
            console.warn('⚠️ [INDEX] No hay áreas en la BD, usando fallback de error');
            areasDatabase = getAreasHardcoded();
        }
        
    } catch (error) {
        console.error('❌ [INDEX] Error al cargar áreas:', error);
        console.warn('⚠️ [INDEX] Usando fallback de error debido a fallo de conexión');
        // Fallback a áreas hardcoded
        areasDatabase = getAreasHardcoded();
    }
}

// ============================================================
// ÁREAS HARDCODED (FALLBACK - SOLO PARA ERRORES DE CONEXIÓN)
// ============================================================

function getAreasHardcoded() {
    console.error('❌ Error de conexión: No se pudieron cargar las áreas desde la base de datos');
    return [
        {
            id: 'error-conexion',
            nombre: '⚠️ Error de Conexión',
            correo: null,
            descripcion: 'No se pudieron cargar las áreas. Por favor, verifica tu conexión e intenta recargar la página.',
...
const ICONO_AREA_POR_DEFECTO = '<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>';

function displayAreasCarousel() {
    const carousel = document.getElementById('areasCarousel');
    
    console.log('🎨 [INDEX] Renderizando carrusel con áreas:', {
        total: areasDatabase.length,
        fuente: areasDatabase.length === 1 && areasDatabase[0].id === 'error-conexion' 
            ? '⚠️ FALLBACK (Error de conexión)' 
            : '✅ BASE DE DATOS',
        areas: areasDatabase.map(a => a.nombre)
    });
    
    // Usar áreas desde BD (o fallback hardcoded)
    carousel.innerHTML = areasDatabase.map(area => {
        // Usar icono personalizado o el icono por defecto
        const iconoMostrar = area.icono || ICONO_AREA_POR_DEFECTO;
        ...
        // Generar HTML del correo si existe
        const correoHTML = area.correo ? `
            <div class="area-email">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                <span>${area.correo}</span>
            </div>
        ` : '';
        ...
        return `
            <div class="area-card" onclick="filterByArea('${area.nombre}')">
                <div class="area-card-header">
                    <div class="area-icon">${iconoMostrar}</div>
                    <h4>${area.nombre}</h4>
                </div>
                <div class="area-card-content">
                    <p>${area.descripcion || 'Accede a los recursos de esta área'}</p>
                    ${correoHTML}
                </div>
